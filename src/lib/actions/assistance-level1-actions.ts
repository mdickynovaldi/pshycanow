...
export async function createAssistanceLevel1(
  ...
  try {
    ...
    return {
      success: false,
      message: "Terjadi kesalahan saat membuat bantuan level 1",
    };
  }
}

// Fungsi untuk mendapatkan bantuan level 1
export async function getAssistanceLevel1(assistanceId: string) {
  const access = await checkAccess();
  ...
  try {
    ...
    return {
      success: false,
      message: "Terjadi kesalahan saat mengambil bantuan level 1",
    };
  }
}
...
export async function submitAssistanceLevel1(
  ...
  try {
    ...
    return {
      success: false,
      message: "Terjadi kesalahan saat mengirimkan bantuan level 1",
    };
  }
}
...
export async function getLatestLevel1Submission(
  ...
  try {
    ...
    const calculatedScore =
      submission.score ?? submission.answers.filter((a) => a.isCorrect).length;
    const calculatedTotalQuestions = submission.assistance.questions.length;

    // Siapkan peta jawaban benar dan penjelasan untuk konsistensi UI
    const correctAnswers: Record<string, boolean> = {};
    const explanations: Record<string, string> = {};
    for (const q of submission.assistance.questions) {
