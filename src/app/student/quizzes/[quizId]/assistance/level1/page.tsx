...
import {
  getLatestLevel1Submission,
  submitAssistanceLevel1,
} from "@/lib/actions/assistance-level1-actions";
...
function formatSubmissionData(apiData: unknown): AssistanceSubmission {
  ...
  if (Array.isArray(data.answers)) {
    data.answers.forEach((ans: unknown) => {
      if (
        ...
        ans !== null &&
        "questionId" in ans &&
        typeof (ans as ApiAnswerItem).questionId === "string" &&
        "answer" in ans &&
        typeof (ans as ApiAnswerItem).answer === "boolean"
      ) {
        formattedAnswers[(ans as ApiAnswerItem).questionId] = (
          ans as ApiAnswerItem
        ).answer;
      }
    });
  ...
  const correctAnswers =
    typeof data.correctAnswers === "object" && data.correctAnswers !== null
      ? (data.correctAnswers as Record<string, boolean>)
      : {};

  const explanations =
    typeof data.explanations === "object" && data.explanations !== null
  ...
  return {
    ...
    scorePercentage:
      typeof data.scorePercentage === "number" ? data.scorePercentage : 0,
...
export default function AssistanceLevel1Page() {
  ...
  const [submission, setSubmission] = useState<AssistanceSubmission | null>(
    null
  );
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        if (
          attemptCountValue >= MAX_ATTEMPTS &&
          quizStatusData.finalStatus !== "PASSED"
        ) {
          setError(
            `Anda telah mencapai batas maksimal percobaan (${MAX_ATTEMPTS} kali). Anda dinyatakan tidak lulus kuis ini.`
          );
          setLoading(false);
          return;
        }
        ...
        console.log(
          "Status bantuan level 1:",
          quizStatusData.assistanceStatus?.level1
        );

        // Periksa apakah assistanceStatus dan level1 ada sebelum mengakses properti
        if (
          !quizStatusData.assistanceStatus ||
          !quizStatusData.assistanceStatus.level1
        ) {
        ...
        if (
          quizStatusData.assistanceStatus.level1.completed &&
          quizStatusData.assistanceStatus.level1.latestSubmission
        ) {
          console.log(
            "Bantuan level 1 sudah diselesaikan, menampilkan hasil terakhir"
          );
        ...
        if (!assistanceResult.success || !assistanceResult.data) {
          ...
          setError(
            assistanceResult.message ||
              "Bantuan level 1 tidak tersedia untuk kuis ini"
          );
          setLoading(false);
          return;
        }
        ...
        console.log(
          "Tipe data bantuan level 1:",
          typeof assistanceResult.data,
        ...
        try {
          ...
          const assistanceData =
            assistanceResult.data as unknown as AssistanceLevel1Response;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const missingAnswers = questionIds.filter(
        (id: string) => answers[id] === undefined
      );

      if (missingAnswers.length > 0) {
        setError(
          `Harap jawab semua pertanyaan (${missingAnswers.length} belum dijawab)`
        );
        setSubmitting(false);
        return;
      }

      const answersPayload: AnswerPayloadItem[] = Object.entries(answers).map(
        ([questionId, answer]) => ({
          questionId,
          answer,
        })
      ...
      if (formattedSubmission.passed) {
        ...
        toast.error("Jawaban Anda belum mencukupi", {
          description:
            "Silakan pelajari petunjuk dan jawaban yang benar, lalu coba lagi",
        });
      }
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Terjadi kesalahan saat mengirimkan jawaban";
      setError(message);
      setSubmitting(false);
    }
  };
  ...
  const completeAssistanceLevel1 = async () => {
    try {
      ...
      toast.error(
        "Terjadi kesalahan saat menyelesaikan bantuan. Silakan coba lagi."
      );
      setSubmitting(false);
    }
  };
  ...
  const continueToMainQuiz = () => {
    if (submission?.passed) {
      ...
      toast.error(
        "Anda harus menjawab semua pertanyaan dengan benar sebelum melanjutkan ke kuis utama."
      );
    }
  };
  ...
  if (
    ...
    !quizStatus?.assistanceStatus.level1.completed
  ) {
    return (
        ...
        <Card>
          ...
          <CardContent>
            <Alert className="mb-4 bg-destructive/15 border-destructive/30">
              ...
              <AlertDescription>
                Anda telah mencapai batas maksimal {MAX_ATTEMPTS} kali
                percobaan. Anda dinyatakan tidak lulus untuk kuis ini.
              </AlertDescription>
            </Alert>

            <p className="text-sm mt-4">
              Silakan hubungi guru Anda untuk informasi lebih lanjut atau
              bantuan tambahan.
            </p>
          </CardContent>
          <CardFooter>
        ...
        {error.includes("tidak tersedia") && (
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg max-w-md text-sm">
            ...
            <p className="text-blue-700 mb-2">
              Bantuan level 1 perlu dikonfigurasi terlebih dahulu melalui panel
              guru. Silakan ikuti langkah-langkah berikut:
            </p>
        ...
        {error.includes("perlu gagal") && (
          <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg max-w-md text-sm">
            ...
            <p className="text-amber-700 mb-2">
              Anda hanya dapat mengakses bantuan level 1 setelah Anda mencoba
              mengerjakan kuis utama terlebih dahulu dan belum memenuhi kriteria
              kelulusan.
            </p>
            <ol className="list-decimal list-inside text-amber-700 space-y-1">
              ...
              <li>
                Jika hasil kuis utama belum memenuhi kriteria lulus, sistem akan
                mengarahkan Anda ke bantuan level 1
              </li>
            </ol>
          </div>
        )}
          ...
          <CardContent>
            ...
            {submission && (
              <div>
                ...
                <p className="text-sm">
                  Skor: {submission.scorePercentage.toFixed(0)}%
                </p>
                <p className="text-sm">
                  Batas Lulus: {submission.passingScore}%
                </p>
                <p className="text-sm">
                  Status: {submission.passed ? "Lulus" : "Tidak Lulus"}
                </p>
              </div>
            )}
          </CardContent>
  ...
  if (submission) {
    ...
    const questionResults =
      assistance?.questions?.map((question: Question) => {
          ...
          <CardContent>
            <div className="mb-4">
              <Alert
                ...
                <AlertDescription>
                  {submission.passed
                    ...
                    ? `Ini adalah kesempatan terakhir Anda (percobaan ke-${
                        attemptCount + 1
                      } dari ${MAX_ATTEMPTS}). Anda harus menjawab semua pertanyaan dengan benar (100%) untuk dapat melanjutkan.`
                    : `Anda harus menjawab semua pertanyaan dengan benar (100%) untuk dapat melanjutkan. Ini percobaan ke-${
                        attemptCount + 1
                      } dari ${MAX_ATTEMPTS}.`}
                </AlertDescription>
              </Alert>
              ...
              <div className="mb-4">
                ...
                <p className="text-sm">
                  Benar: {submission.score} dari {submission.totalQuestions}
                </p>
                <p className="text-sm">
                  Percobaan: {attemptCount} dari {MAX_ATTEMPTS}
                </p>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-medium text-green-800">
                        Selamat! Anda telah lulus bantuan level 1
                      </p>
                      <p className="text-sm text-green-700 mt-1">
                        Anda dapat melanjutkan ke kuis utama
                      </p>
                    </div>
                    <Button
        ...
        <CardContent>
          ...
          <div className="space-y-4">
            {assistance?.questions?.map((question: Question, index: number) => {
              ...
              const correctAnswer =
                isSubmitted && typedSubmission
                  ? typedSubmission.correctAnswers[question.id]
                  : null;
              ...
              const explanation =
                isSubmitted && isWrong && typedSubmission
                  ? typedSubmission.explanations[question.id]
                  : null;
              ...
              return (
                <div
                  ...
                  <div className="flex items-start gap-2">
                    <p className="font-medium mb-3 flex-1">
                      {index + 1}. {question.question}
                    </p>
                    {isWrong && (
                  ...
                  {isWrong && explanation && (
                    <div className="mt-3 text-sm p-2 bg-red-100 border border-red-200 rounded text-red-800">
                      ...
                      <p className="mt-1 font-medium">
                        Jawaban yang benar: {correctAnswer ? "Ya" : "Tidak"}
                      </p>
                    </div>
                  )}
                </div>
